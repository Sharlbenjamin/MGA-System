...
/**
 * System Workflow Documentation Generator
 * 
 * This script automatically generates and maintains the comprehensive
 * System Workflow documentation for the MGA System.
 */
...
class SystemWorkflowGenerator
{
    ...
    /**
     * Load the manifest file to track file changes
     */
    ...
    {
        File::put($this->manifestPath, json_encode($this->manifest, JSON_PRETTY_PRINT));
    }

    /**
     * Check if a file has changed since last generation
     */
    private function hasFileChanged($filePath): bool
    {
        ...
        $storedHash = $this->manifest[$filePath] ?? null;

        if ($storedHash !== $currentHash) {
            $this->manifest[$filePath] = $currentHash;
    ...
    private function getMonitoredFiles(): array
    {
        ...
        foreach ($patterns as $pattern) {
            $files = array_merge($files, glob($this->basePath . '/' . $pattern));
        }
    ...
    public function generate(): void
    {
        ...
        if (!$this->checkForChanges() && File::exists($this->outputPath)) {
    ...
    private function generateVersionsEnvironment(): string
    {
        ...
        $packageJson = json_decode(File::get($this->basePath . '/package.json'), true);
...
if (php_sapi_name() === 'cli') {
    $generator = new SystemWorkflowGenerator();
    $generator->generate();
}
