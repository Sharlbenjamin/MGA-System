...
<body>
    ...
    <p><strong>MGA Reference:</strong> {{ $appointment->file->mga_reference ?? 'mga_reference' }}</p>
    <p><strong>Patient Name:</strong> {{ $appointment->file->patient->name ?? 'patient->name' }}</p>
    ...
    <li><strong>Service:</strong> {{ $appointment->file->serviceType->name ?? 'serviceType->name' }}</li>
    </ul>
    ...
    <p>Dear team,</p>
    <p>We have a patient in {{ $appointment->file->city?->name ?? 'Unknown City' }} that needs a {{ $appointment->file->serviceType->name ?? 'Unknown Service' }} appointment.</p>
